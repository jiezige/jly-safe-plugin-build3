...
static UIViewController *JLYTFTopViewController(void) {
...
static UINavigationController *JLYTFTopNavigationController(void) {
  UIViewController *top = JLYTFTopViewController();
...
static NSDictionary<NSString *, NSString *> *JLYTFRouteParameters(NSString *urlString) {
  if (!urlString.length) {
    return @{};
  }
  NSString *body = urlString;
  NSRange schemeRange = [body rangeOfString:@"://"];
  if (schemeRange.location != NSNotFound) {
    body = [body substringFromIndex:schemeRange.location + schemeRange.length];
  }
...
static void JLYTFSetValueIfPossible(id obj, NSString *key, id value) {
...
static BOOL JLYTFInvokeModuleRouter(NSString *urlString) {
  Class cls = NSClassFromString(@"LE_paradiseModuleRouterHandler");
  if (!cls || !urlString.length) {
    return NO;
  }
  SEL selectors[] = {@selector(moduleRouterWith:), @selector(moduleRouterWithLinkUrl:)};
  for (NSUInteger i = 0; i < sizeof(selectors) / sizeof(SEL); i++) {
    SEL sel = selectors[i];
    if ([cls respondsToSelector:sel]) {
      NSLog(@"[JLYRouteFix] class %@ %@", NSStringFromClass(cls), NSStringFromSelector(sel));
      ((id (*)(id, SEL, id))objc_msgSend)(cls, sel, urlString);
      return YES;
    }
  }
  ...
  for (NSUInteger i = 0; i < sizeof(selectors) / sizeof(SEL); i++) {
    ...
    if ([handler respondsToSelector:sel]) {
      NSLog(@"[JLYRouteFix] instance %@ %@", NSStringFromClass(cls), NSStringFromSelector(sel));
      ((id (*)(id, SEL, id))objc_msgSend)(handler, sel, urlString);
      return YES;
    }
  }
  return NO;
...
static UIViewController *JLYTFCreateFallbackViewController(NSString *urlString) {
  NSString *className = nil;
  NSDictionary *params = JLYTFRouteParameters(urlString);
  ...
  JLYTFSetValueIfPossible(vc, @"routerUrl", urlString);
  JLYTFSetValueIfPossible(vc, @"router_url", urlString);
  JLYTFSetValueIfPossible(vc, @"route_url", urlString);
  JLYTFSetValueIfPossible(vc, @"webViewStringUrl", params[@"path"] ?: urlString);
  JLYTFSetValueIfPossible(vc, @"urlString", params[@"url"] ?: params[@"path"] ?: urlString);
  JLYTFSetValueIfPossible(vc, @"title", params[@"title"]);
  JLYTFSetValueIfPossible(vc, @"target_uid", params[@"target_id"]);
  JLYTFSetValueIfPossible(vc, @"targetUid", params[@"target_id"]);
  JLYTFSetValueIfPossible(vc, @"userID", params[@"target_id"]);
  return vc;
}

static void JLYTFOpenFallbackRoute(NSString *urlString) {
  if (!urlString.length) {
    return;
  }
  NSLog(@"[JLYRouteFix] fallback route %@", urlString);
  if (JLYTFInvokeModuleRouter(urlString)) {
    return;
  }
  UIViewController *vc = JLYTFCreateFallbackViewController(urlString);
  if (!vc) {
    NSLog(@"[JLYRouteFix] no fallback vc for %@", urlString);
    return;
  }
  dispatch_async(dispatch_get_main_queue(), ^{
    UINavigationController *nav = JLYTFTopNavigationController();
    UIViewController *top = JLYTFTopViewController();
    NSLog(@"[JLYRouteFix] open vc=%@ top=%@ nav=%@", JLYTFClassChain(vc), JLYTFClassChain(top), JLYTFClassChain(nav));
    if (nav) {
...
static BOOL JLYTFShouldFallbackRoute(NSString *urlString) {
  if (!urlString.length) {
    return NO;
  }
...
static id JLYTFMakeRouteHandler(NSString *urlString) {
  NSString *capturedURL = [urlString copy];
  void (^handler)(NSDictionary *) = ^(NSDictionary *params) {
    NSLog(@"[JLYRouteFix] handler fired url=%@ params=%@", capturedURL, params);
    JLYTFOpenFallbackRoute(capturedURL);
  };
  return [handler copy];
}
...
static id JLYTF_EVSRouter_instance_handlerForURL(id self, SEL _cmd, id url) {
  id handler = orig_EVSRouter_instance_handlerForURL ? orig_EVSRouter_instance_handlerForURL(self, _cmd, url) : nil;
  NSString *urlString = [url isKindOfClass:[NSURL class]] ? [(NSURL *)url absoluteString] : [url description];
  if (!handler && JLYTFShouldFallbackRoute(urlString)) {
    NSLog(@"[JLYRouteFix] instance missing handler url=%@", urlString);
    return JLYTFMakeRouteHandler(urlString);
  }
  return handler;
}

static id JLYTF_EVSRouter_class_handlerForURL(id self, SEL _cmd, id url) {
  id handler = orig_EVSRouter_class_handlerForURL ? orig_EVSRouter_class_handlerForURL(self, _cmd, url) : nil;
  NSString *urlString = [url isKindOfClass:[NSURL class]] ? [(NSURL *)url absoluteString] : [url description];
  if (!handler && JLYTFShouldFallbackRoute(urlString)) {
    NSLog(@"[JLYRouteFix] class missing handler url=%@", urlString);
    return JLYTFMakeRouteHandler(urlString);
  }
  return handler;
}

static void JLYTFInstallRouteFix(void) {
  Class router = NSClassFromString(@"LE_DL_EVSRouter");
  if (!router) {
    NSLog(@"[JLYRouteFix] LE_DL_EVSRouter not found");
    return;
  }

  unsigned int instanceCount = 0;
  Method *instanceMethods = class_copyMethodList(router, &instanceCount);
  NSMutableArray *instanceNames = [NSMutableArray array];
  for (unsigned int i = 0; i < instanceCount && i < 80; i++) {
    [instanceNames addObject:NSStringFromSelector(method_getName(instanceMethods[i]))];
  }
  free(instanceMethods);
  unsigned int classCount = 0;
  Method *classMethods = class_copyMethodList(object_getClass(router), &classCount);
  NSMutableArray *classNames = [NSMutableArray array];
  for (unsigned int i = 0; i < classCount && i < 80; i++) {
    [classNames addObject:NSStringFromSelector(method_getName(classMethods[i]))];
  }
  free(classMethods);
  NSLog(@"[JLYRouteFix] router methods instance=%@ class=%@", instanceNames, classNames);

  SEL handlerSel = @selector(handlerForURL:);
  Method instanceMethod = class_getInstanceMethod(router, handlerSel);
  if (instanceMethod) {
    orig_EVSRouter_instance_handlerForURL = (id (*)(id, SEL, id))method_getImplementation(instanceMethod);
    method_setImplementation(instanceMethod, (IMP)JLYTF_EVSRouter_instance_handlerForURL);
    NSLog(@"[JLYRouteFix] swizzled -[%@ %@]", NSStringFromClass(router), NSStringFromSelector(handlerSel));
  } else {
    NSLog(@"[JLYRouteFix] missing -[%@ %@]", NSStringFromClass(router), NSStringFromSelector(handlerSel));
  }

  Method classMethod = class_getClassMethod(router, handlerSel);
  if (classMethod) {
    Class meta = object_getClass(router);
    orig_EVSRouter_class_handlerForURL = (id (*)(id, SEL, id))method_getImplementation(classMethod);
    class_replaceMethod(meta, handlerSel, (IMP)JLYTF_EVSRouter_class_handlerForURL, method_getTypeEncoding(classMethod));
    NSLog(@"[JLYRouteFix] swizzled +[%@ %@]", NSStringFromClass(router), NSStringFromSelector(handlerSel));
  } else {
    NSLog(@"[JLYRouteFix] missing +[%@ %@]", NSStringFromClass(router), NSStringFromSelector(handlerSel));
  }
...
static void JLYTFInstall(void) {
  ...
  dispatch_once(&onceToken, ^{
    NSLog(@"[JLYTouchFixV3] loaded conservative mode");
